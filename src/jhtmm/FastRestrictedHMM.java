...
public class FastRestrictedHMM {
    ...
    public void ComputeLogLikelihood(){
        ...
        for (int t = 0; t < numSentences; t++) {
          ...
          logLikelihood += Math.log(norm_factor[t]);
    ...
    void CombineAllProbs(){
        for (int sIndex = 0; sIndex < numSentences; sIndex++) { // sIndex is sentence Index
            double norm = 0;
  ...
  // This method normalizes the vector vec to according to the factor norm.
    ...
    void InitAlpha()  {
        ...
        for (int i = 0; i < numTopics; i++) {
            alpha[0][i] = local[0][i]*pi[i];
            alpha[0][i+numTopics] = local[0][i]*pi[i+numTopics];
            normalizer += alpha[0][i] + alpha[0][i+numTopics];
        }
        Normalize(normalizer, alpha[0]);
  ...
  void InitBeta() {
      for(int ti = 0;ti<numTopics;ti++){
          beta[numSentences-1][ti] = 1;
      }
      Normalize(norm_factor[numSentences-1],beta[numSentences-1]);
  };
  ...
  // The other parameters are as defined above.
  void ComputeAllBetas(){
    for (int sIndex = numSentences-2; sIndex >= 0; sIndex--) {
        double trans_sum = 0;
        for (int ti = 0; ti < numTopics; ti++) {
